...
final class SOMDN_DB_Updater {
	...
	private function update_complete() {
		...
		add_option( $this->setting_name, $this->new_version );
		//echo '<p>Update Complete</p>';
		do_action( 'somdn_update_complete' );
	}

	/**
	 * Checks whether or not this is a clean install, or whether an update is needed.
	 *
	 * @return bool True if updates are needed.
	 */
	private function do_updates() {
		// First we determine if the plugin has ever installed before. If it's a fresh install we don't need to run the updater
		if ( $this->is_clean_install() || $this->is_current_version() ) {
			// Either this is a brand new clean install, or the version numbers are the same. No update necessary.
			return false;
		}
		return true;
	}
